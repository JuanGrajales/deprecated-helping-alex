...
class App extends Component {
  ...
  async componentDidMount() {
    ...
    let latestLaunch = await axios.get(
    ...
    console.log("allLaunches.data", [...allLaunches.data]);
  ...
  findDetails = () => {
    console.log('launch detail mission name:',this.state.match.launch, this.state.allLaunches)
    let theLaunch = this.state.allLaunches.find(eachLaunch => {
        return eachLaunch.mission_name === this.state.match.params.launch
    })
  ...
  showFilteredLaunches = () => {
    // let theDetails = this.findDetails();
    return this.state.filteredLaunches.splice(0, 4).map(eachLaunch => {
      return (
        <div key={eachLaunch.mission_name}>
          <div className="card">
            <div className="card-content">
              <div className="media">
                ...
                <div className="media-content">
                  {console.log("........eachlaunch", eachLaunch.mission_name)}
                  <Link to={`/all-launches/${eachLaunch.mission_name}`}>
                    <p className="title is-4">{eachLaunch.mission_name}</p>
                    <p className="title is-4"></p>
                  </Link>
                  {/* {theDetails.mission_name.map(eachLaunch => (
                        <li key={eachLaunch}>
                            <Link to={`/all-launches/${eachLaunch}`}>
                                {this.state.allLaunches.find(eachLaunch => eachLaunch.mission_name === eachLaunch).name.common}
                            </Link>
                            </li>
                            ))} */}
      ...
      <div>
        <div className="App">
          ...
          <Switch>
            ...
            {/* <Route exact path ="/latest-launch" render={(props) => <LatestLaunch {...props} latestLaunch={this.state.latestLaunch} ready={this.state.ready} allLaunches={this.state.allLaunches}/>}/> */}
            <Route
              exact path={`/all-launches/:launch`}
              render={props => (
                  <PreviousLaunches
                  {...props}
                  allLaunches={this.state.allLaunches}
